...
interface DistribuicaoCategoriasModalProps {
  isOpen: boolean
  onClose: () => void
}

export const DistribuicaoCategoriasModal: React.FC<DistribuicaoCategoriasModalProps> = ({
  isOpen,
  onClose
}) => {
  ...
  const totalProdutos = categoriasData?.reduce((sum, categoria) => sum + categoria.Quantidade, 0) || 0
  const maxValue = categoriasData?.length ? Math.max(...categoriasData.map(categoria => categoria.Quantidade)) : 1
        ...
        {isLoading ? (
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900">Quantidade por Categoria</h2>
              <button
                onClick={onClose}
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Quantidade por Categoria</h2>
                  <p className="text-xs text-gray-500">Distribuição completa por categoria</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="space-y-2">
              {sortedCategorias.map((categoriaData, index) => {
                const { Categoria: categoria, Quantidade: quantidade } = categoriaData
                const percentage = (quantidade / totalProdutos) * 100
                
                return (
                  <div key={categoria} className="flex items-center justify-between py-2 hover:bg-gray-50 rounded-lg px-2 transition-colors">
