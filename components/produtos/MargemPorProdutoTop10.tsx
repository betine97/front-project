...
import { useProdutosTopMargem } from '../../hooks/useProdutosTopMargem';

export function MargemPorProdutoTop10() {
  const { data, loading, error } = useProdutosTopMargem();
  
  console.log('MargemPorProdutoTop10 - loading:', loading, 'error:', error, 'data:', data);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2 pt-4" style={{ minHeight: '140px' }}>
        {currentItems.map((produtoData, index) => {
          const { produto, marca, margem } = produtoData;
          // Pegar apenas as duas primeiras palavras do nome do produto
          const palavras = produto.split(' ');
          const produtoResumido = palavras.slice(0, 2).join(' ') + (palavras.length > 2 ? '...' : '');
            ...
            >
              <div className="flex items-center space-x-2">
                ...
                <div className="min-w-[80px]">
                  <div className="text-sm font-medium text-gray-900" title={`${produto} - ${marca}`}>
                    {produtoResumido}
                  </div>
                </div>
              </div>
